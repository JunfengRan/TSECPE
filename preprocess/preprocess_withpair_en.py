...
df = pd.read_csv('../data/uniconn_2level.csv')
for i in range(len(df)):
    conn = df['conn'][i]
    if conn not in cause_conn:
        cause_conn.append(conn)

# Init csv for recording result
with open ('../data/pairs_en.csv', 'w', newline='') as f:
...
with open ('../data/discourse_en.csv', 'w', newline='') as f:
            csv_writer = csv.writer(f)
            csv_writer.writerow(['section', 'discourse', 'word_count', 'doc_len', 'clause_len', 'ec_emotion_pos', \
                'ec_cause_pos', 'ce_cause_pos', 'ce_emotion_pos', 'ec_true_pairs', 'ce_true_pairs'])
...
with open ('../data/all_data_pair_en.txt', 'r') as f:
    ...
    while sec:
        ...
        with open ('../data/discourse_en.csv', 'a', newline='') as g:
        ...
        for emo_clause_index in ec_emo_index:
            for i in range(length):
                ...
                # Write result in csv
                with open ('../data/pairs_en.csv', 'a', newline='') as g:
                    csv_writer = csv.writer(g)
                    csv_writer.writerow([pair_type, section, emo_clause_index, cau_candidate_index, \
                        emotion_clause, cause_candidate, correctness])
        ...
        for cau_clause_index in ce_cau_index:
            for i in range(length):
                ...
                with open ('../data/pairs_en.csv', 'a', newline='') as g:
                    csv_writer = csv.writer(g)
                    csv_writer.writerow([pair_type, section, cau_clause_index, emo_candidate_index, cause_clause, \
                        emotion_candidate, correctness])
