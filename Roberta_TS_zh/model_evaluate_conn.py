...
class Network(nn.Module):
    ...
    def forward(self, query, query_mask, query_seg, query_len, clause_len, ec_emotion_pos, ec_cause_pos, ce_cause_pos, ce_emotion_pos, doc_len, adj, q_type):
        ...
        pred_emo_cau, ec_conns_list = self.pred_emo_cau(doc_sents_h, query, ec_emotion_pos, doc_len, clause_len)
        pred_cau = self.pred_cau(doc_sents_h)
        pred_cau_emo, ce_conns_list = self.pred_cau_emo(doc_sents_h, query, ce_cause_pos, doc_len, clause_len)
        if q_type == 'emo':
            return pred_emo
        if q_type == 'emo_cau':
            return pred_emo_cau, ec_conns_list
        if q_type == 'cau':
            return pred_cau
        if q_type == 'cau_emo':
            return pred_cau_emo, ce_conns_list
    ...
    def pred_cau_emo(self, doc_sents_h, discourse, cause_pos, doc_len, clause_len):
        ...
        return pred_cau_emo, conns_list # shape: batch_size, 8 * max_doc_len
