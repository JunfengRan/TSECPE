...
class Network(nn.Module):
    ...
    def forward(self, query, query_mask, query_seg, query_len, clause_len, ec_emotion_pos, ec_cause_pos, ce_cause_pos, ce_emotion_pos, doc_len, adj, q_type, ec_true_pairs=None, ce_true_pairs=None):
        ...
        pred_emo_cau, conns_statistics = self.pred_emo_cau(doc_sents_h, query, ec_emotion_pos, doc_len, clause_len, ec_true_pairs)
        pred_cau = self.pred_cau(doc_sents_h)
        pred_cau_emo, conns_statistics = self.pred_cau_emo(doc_sents_h, query, ce_cause_pos, doc_len, clause_len, ce_true_pairs)
        if q_type == 'emo':
            return pred_emo
        if q_type == 'emo_cau':
            return pred_emo_cau, conns_statistics
        if q_type == 'cau':
            return pred_cau
        if q_type == 'cau_emo':
            return pred_cau_emo, conns_statistics
    ...
    def pred_emo_cau(self, doc_sents_h, discourse, emotion_pos, doc_len, clause_len, ec_true_pairs):
        ...
        for i in range(doc_sents_h.size(0)):
                ...
                for k in range(doc_len[i]):
                    ...
                    if ec_true_pairs != None:
                        if [emotion_pos[i][j], k + 1] in ec_true_pairs[i]:
                            if conn in cause_uniconn_token:
                                cause_list[0] += 1
                            else:
                                noncause_list[1] += 1
                                cause_list[2] += 1
                        else:
                            if conn in noncause_uniconn_token:
                                noncause_list[0] += 1
                            else:
                                cause_list[1] += 1
                                noncause_list[2] += 1
            ...
            conns_statistics.append(cause_list)
            conns_statistics.append(noncause_list)
        ...
        return pred_emo_cau, conns_statistics # shape: batch_size, 8 * max_doc_len
    ...
    def pred_cau_emo(self, doc_sents_h, discourse, cause_pos, doc_len, clause_len, ce_true_pairs):
        ...
        return pred_cau_emo, conns_statistics # shape: batch_size, 8 * max_doc_len
