...
class Network(nn.Module):
    ...
    def forward(self, query, query_mask, query_seg, query_len, clause_len, ec_emotion_pos, ec_cause_pos, ce_cause_pos, ce_emotion_pos, doc_len, adj, q_type, ec_true_pairs=None, ce_true_pairs=None):
        ...
        pred_emo_cau = self.pred_emo_cau(doc_sents_h, query, ec_emotion_pos, doc_len, clause_len, ec_true_pairs)
        pred_cau = self.pred_cau(doc_sents_h)
        pred_cau_emo = self.pred_cau_emo(doc_sents_h, query, ce_cause_pos, doc_len, clause_len, ce_true_pairs)
        if q_type == 'emo':
            return pred_emo
        if q_type == 'emo_cau':
            return pred_emo_cau
    ...
    def pred_emo_cau(self, doc_sents_h, discourse, emotion_pos, doc_len, clause_len, ec_true_pairs):
        # For each item in batch
        # shape: batch_size, max_doc_len, feat_dim
        pairs_hs = torch.tensor([]).to(DEVICE)
        max_doc_len = max(doc_len)
        
        if ec_true_pairs != None:
            candidate_conn_true = noncause_uniconn
            candidate_conn_token_true = noncause_uniconn_token
        else:
            candidate_conn_true = candidate_conn
            candidate_conn_token_true = candidate_conn_token
        ...
        for i in range(doc_sents_h.size(0)):
            ...
            for j in range(len(emotion_pos[i])):
                for k in range(doc_len[i]):
                    ...
                    conn = None
                    if ec_true_pairs != None:
                        if [emotion_pos[i][j], k + 1] in ec_true_pairs[i]:
                            conn = '因'
                            conn = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(conn))
                    ...
                    if conn == None:
                        ...
                        if arg1_start != arg2_start:
                            ...
                            # Get conn
                            candidate_conn_score_true = []
                            for idx in range(len(candidate_conn_token_true)):
                                candidate_conn_score_true.extend(mask_distribution[candidate_conn_token_true[idx]].cpu())
                            max_index = torch.argmax(torch.tensor(candidate_conn_score_true))
                            conn = candidate_conn_token_true[max_index]
    ...
    def pred_cau_emo(self, doc_sents_h, discourse, cause_pos, doc_len, clause_len, ce_true_pairs):
        # For each item in batch
        # shape: batch_size, max_doc_len, feat_dim
        pairs_hs = torch.tensor([]).to(DEVICE)
        max_doc_len = max(doc_len)
        
        if ce_true_pairs != None:
            candidate_conn_true = noncause_uniconn
            candidate_conn_token_true = noncause_uniconn_token
        else:
            candidate_conn_true = candidate_conn
        ...
        for i in range(doc_sents_h.size(0)):
            ...
            for j in range(len(cause_pos[i])):
                for k in range(doc_len[i]):
                    ...
                    conn = None
                    if ce_true_pairs != None:
                        if [cause_pos[i][j], k + 1] in ce_true_pairs[i]:
                            conn = '故'
                            conn = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(conn))
