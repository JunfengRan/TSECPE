...
from model_evaluate import Network
import datetime
import numpy as np
import pandas as pd
import pickle
...
def evaluate_one_batch(configs, batch, model, tokenizer):
    ...
    print(section)
    print(pred_emotion_pos)

    # emotion cause step
    cau_pred, conns_list = model(discourse, discourse_mask, segment_mask, query_len, clause_len, pred_emotion_pos, ec_cause_pos, ce_cause_pos, ce_emotion_pos, doc_len, discourse_adj, 'emo_cau')  
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('model/model_conn_fold{}.pth'.format(i))['model'])
