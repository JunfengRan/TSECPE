...
bert = BertModel.from_pretrained(configs.bert_cache_path).to(DEVICE)
...
class Network(nn.Module):
    ...
    def forward(self, query, query_mask, query_seg, query_len, clause_len, ec_emotion_pos, ec_cause_pos, ce_cause_pos, ce_emotion_pos, doc_len, adj, conn, q_type):
        ...
        pred_ec_emo_cau = self.ec_pred_emo_cau(doc_sents_h, ec_emotion_pos, doc_len, conn, ec_pair_count, ce_pair_count)
        pred_ce_cau = self.ce_pred_cau(doc_sents_h)
        pred_ce_cau_emo = self.ce_pred_cau_emo(doc_sents_h, ce_cause_pos, doc_len, conn, ec_pair_count, ce_pair_count)
        if q_type == 'ec_emo':
            return pred_ec_emo
...
class Predictions_ec_emo_cau(nn.Module):
    ...
    def forward(self, doc_sents_h, emotion_pos, doc_len, conn, ec_pair_count, ce_pair_count):
...
class Predictions_ce_cau_emo(nn.Module):
    ...
    def forward(self, doc_sents_h, cause_pos, doc_len, conn, ec_pair_count, ce_pair_count):
        doc_sents_h_2d = doc_sents_h.squeeze(0)  # shape: batch_size=1, max_doc_len, 768, squeeze dim0
        # Set single mask for one connective
        mask = torch.tensor([1] + [0] * 511).unsqueeze(0)
        segement = torch.tensor([0] * 512).unsqueeze(0)
