...
class Discourse(Dataset):
    def __init__(self, tokenizer, path, start, end):
        ...
        for i in range(len(df)):
            if int(df['section'][i]) < start or int(df['section'][i]) >= end:
                continue
...
if __name__ == '__main__':
    ...
    train_start = int(0 * dataset_len) + 1
    train_end = int(0.9 * dataset_len) + 1
    train_dataset = Discourse(tokenizer, configs.dataset_path, train_start, train_end)
    ...
    test_start = int(0.9 * dataset_len) + 1
    test_end = int(1 * dataset_len) + 1
