...
class Network(nn.Module):
    def __init__(self, configs):
        ...
        self.bert_train = AutoModel.from_pretrained(self.configs.bert_cache_path)
        self.bert_mask = AutoModelForMaskedLM.from_pretrained(self.configs.bert_cache_path)
        self.bert_eval = AutoModel.from_pretrained(self.configs.bert_cache_path)
        self.tokenizer = AutoTokenizer.from_pretrained(self.configs.bert_cache_path)
            ...
            for j in range(len(emotion_pos[i])):
                for k in range(doc_len[i]):
                    ...
                    if arg1_start != arg2_start:
                        ...
                        with torch.no_grad():
                            mask_distribution = self.bert_mask(inputs.to(DEVICE), mask.to(DEVICE), segement.to(DEVICE))[0][0][len1 + 1]
                        ...
                        min_index = torch.argmin(torch.tensor(candidate_conn_score))
                        conn = candidate_conn_token[min_index]
