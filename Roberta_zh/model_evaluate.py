...
class Network(nn.Module):
    ...
    def forward(self, query, query_mask, query_seg, query_len, clause_len, emotion_pos, cause_pos, doc_len, adj, q_type, true_pairs=None):
        ...
        pred_emo_cau, conns_list = self.pred_emo_cau(doc_sents_h, query, emotion_pos, doc_len, clause_len, true_pairs)
        if q_type == 'emo':
            return pred_emo
        if q_type == 'emo_cau':
    ...
    def pred_emo_cau(self, doc_sents_h, discourse, emotion_pos, doc_len, clause_len, true_pairs):
        ...
        for i in range(doc_sents_h.size(0)):
            ...
            for j in range(len(emotion_pos[i])):
                for k in range(doc_len[i]):
                    ...
                    conn = None
                    if true_pairs != None:
                        if [emotion_pos[i][j], k + 1] in true_pairs[i]:
                            conn = '因'
