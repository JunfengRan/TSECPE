...
def evaluate_one_batch(configs, batch, model, tokenizer):
    ...
    # emotion cause step
    cau_pred, conns_list = model(discourse, discourse_mask, segment_mask, query_len, clause_len, pred_emotion_pos, ec_cause_pos, doc_len, discourse_adj, 'emo_cau', ec_true_pairs)
...
def main(configs, train_loader, test_loader, tokenizer):
    ...
    model.load_state_dict(torch.load('model/model_conn_true_save.pth')['model'])
